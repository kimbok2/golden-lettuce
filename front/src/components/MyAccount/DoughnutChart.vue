<template>
  ...
  >
    <RouterLink :to="{ name: 'profile' }" class="custom-link">
      <div>
        더 많은 상품을 추천받으시려면 내 정보를 입력해주세요
      </div></RouterLink
    >
  </div>
</template>
...
const animateText = (start, end, duration) => {
  ...
  const step = (currentTime) => {
    const progress = Math.min((currentTime - startTime) / duration, 1);
    chartOptions.value.plugins.centerText.text = `${Math.floor(
...
.custom-link div:hover {
  color: grey; /* 마우스 오버 시 색상 */
}
</style>
